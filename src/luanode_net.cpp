...
Socket::~Socket(void)
{
	...
	if(m_socket->is_open()) {
		...
		m_socket->close(ec);
		if(ec) {
			LogError("Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
}
...
void Socket::HandleWrite(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_writes--;
	if(!error) {
		LogInfo("Socket::HandleWrite (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		...
		LogDebug("Socket::HandleWrite with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		lua_getfield(L, 1, "write_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			...
			LogError("Socket::HandleWrite with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		}
	}
	lua_settop(L, 0);

	if(m_write_shutdown_pending && m_pending_writes == 0) {
		LogDebug("Socket::HandleWrite - Applying delayed send shutdown (%p) (id:%lu) (send)", this, m_socketId);
		boost::system::error_code ec;
		m_socket->shutdown(boost::asio::socket_base::shutdown_send, ec);
		if(ec) {
			LogError("Socket::HandleWrite - Error shutting down socket (%p) (id:%lu) (send) - %s", this, m_socketId, ec.message().c_str());
		}
	}
	...
	if(m_close_pending && m_pending_writes == 0 && m_pending_reads == 0) {
		...
		m_socket->close(ec);
		if(ec) {
			LogError("Socket::HandleWrite - Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
}
...
void Socket::HandleReadDelimited(int reference, const std::string& delimiter, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_reads--;
	if(!error) {
		LogInfo("Socket::HandleReadDelimited (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		...
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			...
			if(lua_type(L, 1) == LUA_TUSERDATA) {
				userdataType* ud = static_cast<userdataType*>(lua_touserdata(L, 1));
				LogWarning("Socket::HandleReadDelimited (%p) (id:%lu) - No read_callback set on %s (address: %p, possible obj: %p)", this, m_socketId, luaL_typename(L, 1), ud, ud->pT);
			}
			else {
				LogWarning("Socket::HandleReadDelimited (%p) (id:%lu) - No read_callback set on %s", this, m_socketId, luaL_typename(L, 1));
			}
		}
	}
	else {
		LogDebug("Socket::HandleReadDelimited with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			BoostErrorCodeToLua(L, error);	// -> nil, error message, error code
			...
			LogError("Socket::HandleReadDelimited with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		}
	}
	lua_settop(L, 0);
	...
	if(m_close_pending && m_pending_writes == 0 && m_pending_reads == 0) {
		...
		m_socket->close(ec);
		if(ec) {
			LogError("Socket::HandleReadDelimited - Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
}
...
void Socket::HandleReadSome(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_reads--;
	if(!error) {
		LogInfo("Socket::HandleReadSome (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			...
			if(lua_type(L, 1) == LUA_TUSERDATA) {
				userdataType* ud = static_cast<userdataType*>(lua_touserdata(L, 1));
				LogWarning("Socket::HandleReadSome (%p) (id:%lu) - No read_callback set on %s (address: %p, possible obj: %p)", this, m_socketId, luaL_typename(L, 1), ud, ud->pT);
			}
			else {
				LogWarning("Socket::HandleReadSome (%p) (id:%lu) - No read_callback set on %s", this, m_socketId, luaL_typename(L, 1));
			}
		}
	}
	else {
		...
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			...
			switch(error.value()) {
				...
				case boost::asio::error::connection_reset:
					LogDebug("Socket::HandleReadSome (connection reset) (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
				break;
				default:
					LogError("Socket::HandleReadSome with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
				break;
			}
			LuaNode::GetLuaVM().call(4, LUA_MULTRET);
	...
	if(m_close_pending && m_pending_writes == 0 && m_pending_reads == 0) {
		...
		m_socket->close(ec);
		if(ec) {
			LogError("Socket::HandleReadSome - Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
}
...
void Socket::HandleReadSize(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_reads--;
	if(!error) {
		LogInfo("Socket::HandleReadSize (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			const char* data = (const char*)boost::asio::detail::buffer_cast_helper(m_inputBuffer.data());
			...
			if(lua_type(L, 1) == LUA_TUSERDATA) {
				userdataType* ud = static_cast<userdataType*>(lua_touserdata(L, 1));
				LogWarning("Socket::HandleReadSize (%p) (id:%lu) - No read_callback set on %s (address: %p, possible obj: %p)", this, m_socketId, luaL_typename(L, 1), ud, ud->pT);
			}
			else {
				LogWarning("Socket::HandleReadSize (%p) (id:%lu) - No read_callback set on %s", this, m_socketId, luaL_typename(L, 1));
			}
		}
	}
	else {
		...
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			...
			switch(error.value()) {
				case boost::asio::error::eof:
					LogDebug("Socket::HandleReadSize (EOF) (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
					break;
				case boost::asio::error::operation_aborted:
					LogDebug("Socket::HandleReadSize (operation aborted) (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
					break;
				case boost::asio::error::connection_reset:
					LogDebug("Socket::HandleReadSize (connection reset) (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
					break;
				default:
					LogError("Socket::HandleReadSize with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
					break;
			}
	...
	if(m_close_pending && m_pending_writes == 0 && m_pending_reads == 0) {
		...
		m_socket->close(ec);
		if(ec) {
			LogError("Socket::HandleReadSize - Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
}
