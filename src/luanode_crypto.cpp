...
{
	s_socketCount++;
	LogDebug("Constructing Crypto::Socket (%p) (id:%lu). Current socket count = %lu", this, m_socketId, s_socketCount);
...
Socket::~Socket(void)
{
	...
	LogDebug("Destructing Crypto::Socket (%p) (id:%lu). Current socket count = %lu", this, m_socketId, s_socketCount);
}
...
int Socket::DoHandShake(lua_State* L) {
	LogDebug("SecureSocket::DoHandShake (%p) (id:%lu)", this, m_socketId);
...
void Socket::HandleHandshake(int reference, const boost::system::error_code& error) {
	...
	if(!error) {
		LogInfo("SecureSocket::HandleHandshake (%p) (id:%lu)", this, m_socketId);
		...
		LogDebug("SecureSocket::HandleHandshake with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		lua_getfield(L, 1, "handshake_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			...
			LogError("SecureSocket::HandleWrite with error (%p) - (id:%lu) - %s", this, m_socketId, error.message().c_str());
		}
	}
	lua_settop(L, 0);
}
...
int Socket::Write(lua_State* L) {
	...
	if(lua_type(L, 2) == LUA_TSTRING) {
		...
		luaL_error(L, "SecureSocket::Write (%p) (id:%lu), unhandled type '%s'", this, m_socketId, luaL_typename(L, 2));
	}
	lua_pushboolean(L, true);
	return 1;
}
...
void Socket::HandleWrite(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_writes--;
	if(!error) {
		LogInfo("SecureSocket::HandleWrite (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		...
		LogDebug("SecureSocket::HandleWrite with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		lua_getfield(L, 1, "write_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			BoostErrorCodeToLua(L, error);	// -> nil, error code, error message
	...
	if(m_pending_writes == 0 && m_shutdown_pending) {
		...
		if(ec) {
			LogWarning("Error en shutdown (%p) (id:%lu) '%s'", this, m_socketId, ec.message().c_str());
		}
	}
	...
	if(m_close_pending && m_pending_writes == 0 && m_pending_reads == 0) {
		...
		if(ec) {
			LogError("SecureSocket::HandleWrite - Error closing socket (%p) (id:%lu) - %s", this, m_socketId, ec.message().c_str());
		}
	}
...
void Socket::HandleRead(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_reads--;
	if(!error) {
		LogInfo("SecureSocket::HandleRead (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		...
		LogDebug("SecureSocket::HandleRead with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			BoostErrorCodeToLua(L, error);	// -> nil, error code, error message
			...
			LogError("SecureSocket::HandleRead with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		}
	}
...
void Socket::HandleReadSome(int reference, const boost::system::error_code& error, size_t bytes_transferred) {
	...
	m_pending_reads--;
	if(!error) {
		LogInfo("SecureSocket::HandleReadSome (%p) (id:%lu) - Bytes Transferred (%d)", this, m_socketId, bytes_transferred);
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
...
int Socket::Close(lua_State* L) {
	...
	if(m_pending_writes) {
		LogDebug("SecureSocket::Close - Socket (%p) (id:%lu) marked for closing", this, m_socketId);
	...
	LogDebug("SecureSocket::Close - Socket (%p) (id:%lu) closing now", this, m_socketId);
	boost::system::error_code ec;
	m_ssl_socket->lowest_layer().close(ec);
	return BoostErrorCodeToLua(L, ec);
}
...
int Socket::Shutdown(lua_State* L) {
	LogDebug("SecureSocket::Shutdown (%p) (id:%lu)", this, m_socketId);
...
void Socket::HandleShutdown(int reference, const boost::system::error_code& error) {
	...
	if(!error) {
		LogInfo("SecureSocket::HandleShutdown (%p) (id:%lu)", this, m_socketId);
		...
		LogDebug("SecureSocket::HandleShutdown with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		//lua_getfield(L, 1, "shutdown_callback");
		lua_getfield(L, 1, "read_callback");
		if(lua_type(L, 2) == LUA_TFUNCTION) {
			lua_pushvalue(L, 1);
			...
			LogError("SecureSocket::HandleShutdown with error (%p) (id:%lu) - %s", this, m_socketId, error.message().c_str());
		}
	}
