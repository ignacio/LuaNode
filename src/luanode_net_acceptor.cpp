...
{
	s_acceptorCount++;
	LogDebug("Constructing Acceptor (%p) (id:%lu). Current acceptor count = %lu", this, m_acceptorId, s_acceptorCount);
}

Acceptor::~Acceptor(void)
{
	s_acceptorCount--;
	LogDebug("Destructing Acceptor (%p) (id:%lu). Current acceptor count = %lu", this, m_acceptorId, s_acceptorCount);
	...
	if(m_acceptor.is_open()) {
		...
		m_acceptor.close(ec);
		if(ec) {
			LogError("Error closing acceptor (%p) (id:%lu) - %s", this, m_acceptorId, ec.message().c_str());
		}
	}
}
...
/*static*/ int Acceptor::tostring_T(lua_State* L) {
	...
	Acceptor* obj = ud->pT;
	lua_pushfstring(L, "%s (%p) (id:%lu)", className, obj, obj->m_acceptorId);
	return 1;
}
...
int Acceptor::Open(lua_State* L) {
	...
	LogDebug("Acceptor::Open (%p) (id:%lu) - %s", this, m_acceptorId, kind);
...
int Acceptor::Close(lua_State* L) {
	LogDebug("Acceptor::Close (%p) (id:%lu)", this, m_acceptorId);
	boost::system::error_code ec;
	m_acceptor.close(ec);
	return BoostErrorCodeToLua(L, ec);
}
...
int Acceptor::SetOption(lua_State* L) {
	...
	LogDebug("Acceptor::SetOption (%p) (id:%lu) - %s", this, m_acceptorId, option);
...
int Acceptor::Bind(lua_State* L) {
	...
	LogDebug("Acceptor::Bind (%p) (id:%lu) - (%s,%d)", this, m_acceptorId, ip, port);
...
int Acceptor::Listen(lua_State* L) {
	...
	LogDebug("Acceptor::Listen (%p) (id:%lu) - backlog = %d", this, m_acceptorId, backlog);
...
int Acceptor::Accept(lua_State* L) {
	LogDebug("Acceptor::Accept (%p) (id:%lu)", this, m_acceptorId);
...
void Acceptor::HandleAccept(int reference, boost::shared_ptr<boost::asio::ip::tcp::socket> socket, const boost::system::error_code& error) {
	LogDebug("Acceptor::HandleAccept (%p) (id:%lu) (new socket %p)", this, m_acceptorId, socket.get());
	...
	if(!error) {
		...
		if(error != boost::asio::error::operation_aborted) {
			LogError("Acceptor::HandleAccept (%p) (id:%lu) (new socket %p) - %s", this, m_acceptorId, socket.get(), error.message().c_str());
		}
	}
	lua_settop(L, 0);
}
